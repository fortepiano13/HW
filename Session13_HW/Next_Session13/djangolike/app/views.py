...
def scrap(request):
    ...
    scrap_1 = Scrap.objects.filter(
        post=Post.objects.get(pk=post_pk)
    )

    response = {
        'scrap_count': scrap_1.count()
    }
    return HttpResponse(json.dumps(response))
...
def mypage(request):
    ...
    like_1 = likes.filter(user=request.user)

    scraps = Scrap.objects.all()
    scrap_2 = scraps.filter(user=request.user)

    return render(request, 'mypage.html', { "posts":posts, "like_1":like_1, "scrap_2": scrap_2  })
